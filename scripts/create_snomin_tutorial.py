...
cells = [
    md(
        ...
        "\n"
        "> **Colab:** run the install cell once — it restarts the runtime automatically. "
    ...
    code(
        ...
        "setup_marker = repo_root / '.colab_setup_done'\n"
        "\n"
        "if IN_COLAB:\n"
        ...
        "        # Rebuild NumPy first to avoid ABI mismatch with Colab preinstalled packages\n"
        "        subprocess.run([*pip, '--force-reinstall', 'numpy==1.26.4'], check=True)\n"
        "        deps = [\n"
        ...
        "        subprocess.run([*pip, '-e', str(repo_root)], check=True)\n"
        "        # Rebuild compiled deps against the new NumPy\n"
        "        subprocess.run(\n"
        "            [*pip, '--force-reinstall', 'numpy==1.26.4', 'scipy', 'pandas'],\n"
        "            check=True,\n"
        "        )\n"
